...
- 최소한 success/fail + error_code + 실패 이유 텍스트 포함
...
- TODO: `docs/OPEN_QUESTIONS.md`에 정책안 확정 전까지 후보 규칙 기록

## 3) Token Budget 권장안 (qwen3-vl-32b, 1 FPS 목표)

원칙:
- Verify 루프(1초 이내)는 작고 고정된 컨텍스트
- Planner는 조금 더 길게, 대신 트리거 기반 요약으로만 갱신

권장 시작값:
- **Verify Context:** 2k~3k tokens
  - 직전 action + executor_result + 최신 observation + 핵심 facts만 포함
...
이유:
- 1 FPS 루프에서 매 스텝 긴 프롬프트는 지연과 비용을 급격히 증가시킴
- Verify는 "정답/오답 판단" 중심이라 정보량이 적어도 됨
- 장기 계획은 summary 기반으로 안정적으로 유지
